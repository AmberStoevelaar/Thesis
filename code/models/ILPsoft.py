...
class ObjectiveLogger:
    def __init__(self, results_folder, timestamp, timelimit):
        ...
        self.log_file_path = os.path.join(log_folder, f"ILPSOFT_{self.timestamp}.csv")
        ...
        with open(self.log_file_path, mode='w', newline='') as file:
            ...
            writer.writerow(["Method", "ILPSOFT"])
...
def run_ilp_soft(school, processed_data_folder, timelimit):
    model, x = create_model(school, processed_data_folder)

    # Define paths
    folder = 'data/results'
    timestamp = datetime.now().strftime("%d-%m_%H:%M")
    results_folder = os.path.join(folder, school, "ILPSOFT")
