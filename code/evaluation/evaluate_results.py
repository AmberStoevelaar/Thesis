...
def compute_normalized_balance_deviation(balance_dict, group_sizes):
    ...
    for attribute, values in balance_dict.items():
        total_deviation = 0
        for val_name, details in values.items():
            total = details["total"]
            if total == 0:
...
def only_minimum_satisfied(df):
    ...
    count_min = 0              # count of students with exactly min_satisfied preferences met
    count_above_min = 0        # count of students with more than min_satisfied preferences met
    eligible_students = 0      # students who submitted more than min_satisfied preferences
...
def run_evaluate(school, processed_data_folder, method, groups, timestamp):
    ...
    print(f"Students ONLY minimum preferences satisfied: {evaluation_results['only_minimum_satisfied']}")
    print(f"Percentage ONLY minimum preferences satisfied: {evaluation_results['only_minimum_percentage']:.2%}")
    print(f"Students ABOVE minimum preferences satisfied: {evaluation_results['more_than_minimum_satisfied']}")
    print(f"Percentage ABOVE minimum preferences satisfied: {evaluation_results['more_than_minimum_percentage']:.2%}")
    ...
    print("Balance deviation:", evaluation_results["balance_deviation"])
    print("Total deviation:", evaluation_results["total_deviation"])
